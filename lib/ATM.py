...
redisConnection = {"host": "localhost", "port": 6379, "decode_responses": True}
...
class ATM:
    ...
    def login(self, user: str, password: int) -> int:
        ...
        with self.__mysqlDB.cursor(prepared=True, dictionary=True) as cursor:
            cursor.execute(
                "SELECT ID, Holder, Balance FROM `Accounts` WHERE `Holder` = %s AND `PIN` = %s",
                (user, password),
            )
    ...
    def authenticateLogin(self, otp: int) -> bool:
        ...
        self._isAuthenticated = (
            int(str(self.__redisDB.get(self.currentUser["ID"]))) == otp
        )
        return self._isAuthenticated
    ...
    def withdraw(self, amount: float) -> dict:
        ...
        try:
            ...
            raise ConnectionError(
                "Something Wrong With The Database, Guess Whatever You Are Doing Should Wait"
            )
        else:
    ...
    def getTransactionList(self) -> list[tuple]:
        """
        Returns All Transactions of the user as list of tuples
        """
        with self.__mysqlDB.cursor(prepared=True) as cursor:
            cursor.execute(
                "SELECT TID, TransactTime, TransactType, Amount, Balance FROM `Transactions` WHERE AccountID = %s",
                [self.currentUser["ID"]],
            )
...
if __name__ == "__main__":
    atmTest = ATM()
    atmTest.login("ARG", 694201)
    from rich import print

    print(atmTest.getTransactionList())
